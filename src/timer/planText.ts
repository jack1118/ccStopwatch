...
// 每段距離標註：p=每圈配速秒、r=間休秒（皆可省）。例 400m p84s r90s
function itemTokens(it: Item, lapMeters: number): string {
  const p = it.targetSec && it.targetSec > 0 ? ` p${Math.round((it.targetSec * lapMeters) / it.meters)}s` : ''
  const r = it.restSec > 0 ? ` r${it.restSec}s` : ''
  return p + r
}

/** 單一：600m×10 p96s r90s；組合：(400m p84s r90s+200m r60s)×8 */
export function segLabel(seg: Segment, lapMeters: number): string {
  const items = itemsOf(seg)
  return items.length > 1
    ? `(${items.map((i) => `${i.meters}m${itemTokens(i, lapMeters)}`).join('+')})×${seg.reps}`
    : `${items[0].meters}m×${seg.reps}${itemTokens(items[0], lapMeters)}`
}

export function planSummary(segments: Segment[], lapMeters: number): string {
  return segments.map((s) => segLabel(s, lapMeters)).join(' ')
}
...
function parseItem(raw: string, lapMeters: number, gapSec: number): Item | null {
  const m = raw.match(/(\d+)\s*m/i)
  const meters = m ? Number(m[1]) : 0
  if (!meters) return null
  const p = raw.match(/p\s*(\d+)\s*s?/i)
  const r = raw.match(/r\s*(\d+)\s*s?/i)
  const pace = p ? Number(p[1]) : 0
  return {
    id: uid(),
    meters,
    restSec: r ? Number(r[1]) : 0,
    targetSec: pace > 0 ? Math.round((pace * meters) / lapMeters) : 0,   // p=每圈配速 → 整段目標秒
    gapSec,
  }
}
...
export function parsePlan(text: string, lapMeters: number): Segment[] | null {
  ...
  while ((mm = segRe.exec(s)) !== null) {
    ...
    if (mm[1] != null) {
      const items = mm[1].split('+').map((part) => parseItem(part, lapMeters, gapSec))
      if (items.some((it) => !it)) return null
      segs.push({ id: uid(), reps: Number(mm[2]), items: items as Item[] })
    } else {
      const it = parseItem(`${mm[3]}m ${mm[5] ?? ''}`, lapMeters, gapSec)
      if (!it) return null
